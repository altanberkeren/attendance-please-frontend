...
function QRDisplay() {
  ...
  const refresh = useCallback(() => {
    setToken(
      `att://session?base=${encodeURIComponent(baseToken)}&ts=${Date.now()}`,
    );
  }, [baseToken]);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center pt-12 pb-0 px-8 select-none">
      ...
      <div className="flex items-center gap-2.5 flex-wrap justify-center mb-24">
        <span className="text-white/90 text-lg font-mono font-bold">
          {course}
        </span>
        {module && (
          <>
            <span className="text-white/30">·</span>
            <span className="text-white/60 text-base">{module}</span>
          </>
        )}
        {section && (
          <>
            <span className="text-white/30">·</span>
            <span className="text-white/60 text-base">{section}</span>
          </>
        )}
      </div>
