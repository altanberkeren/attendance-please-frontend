...
import {
  ...
  CalendarCheck, ChevronRight, MoreHorizontal, X,
} from "lucide-react"
...
import { MOCK_TERMS, Term } from "@/lib/mock/terms"
  ...
  return (
    <div className="rounded-lg border border-dashed border-primary/40 bg-primary/5 p-3 space-y-2">
      ...
      <Button
        ...
        disabled={!valid}
        onClick={() =>
          onSave({ id: Date.now().toString(), name: title.trim(), code: code.trim().toUpperCase(), description: "" })
        }
      >
        Add Course
      </Button>
...
function InlineTermForm({
  ...
  return (
    <div className="rounded-lg border border-dashed border-primary/40 bg-primary/5 p-3 space-y-3">
      ...
      <div className="space-y-1">
        <Label className="text-xs">Term Name <span className="text-destructive">*</span></Label>
        <Input
          className="h-8 text-sm"
          value={name}
      ...
      <Button
        ...
        disabled={!valid}
        onClick={() =>
          onSave({ id: Date.now().toString(), name: name.trim(), startDate, endDate })
        }
      >
        Add Term
      </Button>
...
function CreateOfferingDialog({
  ...
  const courseOptions = courses.map((c) => ({ value: c.id, label: c.name, sublabel: c.code }))
  const termOptions   = terms.map((t) => ({
    value: t.id,
    label: t.name,
    sublabel: t.startDate && t.endDate ? `${t.startDate} → ${t.endDate}` : undefined,
  }))
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{editing ? "Edit Offering" : "New Course Offering"}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-1">
          ...
          <div className="space-y-2">
            ...
            <CreatableCombobox
              ...
              onCreate={(name) => { setCourseForm(name); setTermForm(null) }}
            ...
            {selectedCourse && !courseForm && (
              <p className="text-xs text-muted-foreground">
                Code: <span className="font-mono font-medium">{selectedCourse.code}</span>
              </p>
            )}
            {courseForm !== null && (
              <InlineCourseForm
          ...
          <div className="space-y-2">
            ...
            <CreatableCombobox
              ...
              onCreate={(name) => { setTermForm(name); setCourseForm(null) }}
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!courseId || !termId || courseForm !== null || termForm !== null}>
            {editing ? "Save" : "Create"}
          </Button>
        </DialogFooter>
      </DialogContent>
...
export default function CourseOfferingsPage() {
  ...
  function handleCourseCreate(c: Course) { setCourses((p) => [...p, c]) }
  function handleTermCreate(t: Term)     { setTerms((p) => [...p, t]) }

  function handleSubmit(courseId: string, termId: string) {
    const course = courses.find((c) => c.id === courseId)!
    const term   = terms.find((t) => t.id === termId)!
    if (editing) {
      setOfferings((p) => p.map((o) =>
        o.id === editing.id ? { ...o, courseId, termId, courseName: course.name, termName: term.name } : o
      ))
    } else {
      setOfferings((p) => [...p, {
        id: Date.now().toString(), courseId, termId,
        courseName: course.name, termName: term.name,
        section: "A", students: [], staff: [], sessions: [],
      }])
    }
  }
      ...
      {termOrder.map((term) => (
        <div key={term} className="space-y-3">
          <div className="flex items-center gap-3">
            ...
            <Badge variant="secondary" className="text-xs">{grouped[term].length} offering{grouped[term].length !== 1 ? "s" : ""}</Badge>
              ...
              <Card key={offering.id} className="group hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold font-mono ${sectionColor(offering.section)}`}>
                      Section {offering.section}
                    </span>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="h-6 w-6 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => handleDelete(offering.id)}>
                  ...
                  <p className="font-semibold text-sm leading-snug mb-3 line-clamp-2">{offering.courseName}</p>

                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{offering.students.length} students</span>
                    <span className="flex items-center gap-1"><UserCog className="h-3.5 w-3.5" />{offering.staff.length} staff</span>
                    <span className="flex items-center gap-1"><CalendarCheck className="h-3.5 w-3.5" />{offering.sessions.length} sessions</span>
                  </div>

                  <div className="flex gap-2 mt-3 pt-3 border-t opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                    <Button variant="default" size="sm" className="flex-1 h-7 text-xs gap-1.5" onClick={() => router.push(`/course-offerings/${offering.id}`)}>
                      <ChevronRight className="h-3 w-3" />Details
                    </Button>
                    <Button variant="outline" size="sm" className="h-7 text-xs px-2" onClick={() => openEdit(offering)}>
                      <Pencil className="h-3 w-3" />
                    </Button>
                    <Button variant="outline" size="sm" className="h-7 text-xs px-2 text-destructive hover:text-destructive border-destructive/30 hover:bg-destructive/5" onClick={() => handleDelete(offering.id)}>
