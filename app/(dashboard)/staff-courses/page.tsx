...
function computeModulePcts(matrix: AttendanceMatrixResult) {
  return matrix.modules.map((mod, i) => {
    ...
    const present = matrix.students.filter(
      (s) =>
        s.attendanceStatuses[i] === AttendanceStatus.Present ||
  ...
  return (
    <div>
      <p className="text-xs text-muted-foreground mb-3">
        Last session · <span className="font-medium">{last.moduleTitle}</span>
...
function CourseOfferingCard({
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        className="flex w-full items-center gap-4 p-5 text-left cursor-pointer select-none"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            <span className={cn("font-mono font-bold text-base", colors.text)}>
              {offering.courseCode}
            </span>
          ...
          <div className="text-center">
            ...
            <p
              className={cn(
                ...
                overallPct !== null ? textColor(overallPct) : "",
              )}
            >
          ...
          <div className="p-5 space-y-5">
                          ...
                          {sections.length === 0 ? (
                            ...
                            sections.map((sec) => {
                              const count = matrix.students.filter(
                                (s) =>
                                  String(s.currentSectionId) === String(sec.id),
                              ).length;
                              return (
                                <button
                                  key={String(sec.id)}
                                  ...
                                  onClick={() =>
                                    router.push(
                                      courseOfferingHref(offering.id, {
                                        sectionId: sec.id,
                                      }),
                                    )
                                  }
            ...
            <div className="flex items-center gap-2 pt-1 border-t">
              <Button
                ...
                variant="default"
                className="gap-2"
                onClick={() => router.push(courseOfferingHref(offering.id))}
              >
              ...
              <Button
                ...
                onClick={() =>
                  router.push(
                    courseOfferingHref(offering.id, { tab: "sessions" }),
                  )
                }
              >
...
export default function MyCoursesPage() {
  ...
  const { data: realOfferings = [], isLoading: loadingOfferings } =
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {isLoading
              ? "Loading…"
              : `${myOfferings.length} course${myOfferings.length !== 1 ? "s" : ""} assigned`}
          </p>
        </div>
        <Button
          onClick={() => router.push("/attendance")}
