...
function calcRate(data: ClassData): number {
  const total =
    data.presentCount + data.lateCount + data.absentCount + data.excusedCount;
  if (total === 0) return 0;
  return Math.round(
    ((data.presentCount + data.lateCount + data.excusedCount) / total) * 100,
  );
}
...
function calcAbsencesRemaining(data: ClassData): number {
  ...
  const maxMisses = Math.floor(
    data.totalModules * ((100 - FAIL_THRESHOLD) / 100),
  );
      ...
      {data.modules.map((module) => (
        ...
        >
          ...
          <div className="flex shrink-0 items-center gap-2">
            ...
            <Badge
              variant="outline"
              className={cn("text-xs", statusStyles(module.status))}
            >
            ...
            {remaining <= 1 && data.totalModules > 0 ? (
              <div className="flex items-center gap-2 rounded-lg border border-yellow-500/20 bg-yellow-500/10 p-3 text-yellow-700 dark:text-yellow-400">
                ...
                <span className="text-xs">
                  Attendance is close to the {FAIL_THRESHOLD}% threshold.
                </span>
              </div>
            ) : null}
          </CardContent>
        ...
        <TabsContent value="summary" className="mt-4">
          <Card>
            <CardContent className="space-y-3 pt-4 text-sm text-muted-foreground">
              <p>
                This view uses your enrolled course offerings and attendance
                overview from the backend.
              </p>
              <p>
                Detailed calendar/date support can be added once session dates
                are exposed for students.
              </p>
            </CardContent>
          </Card>
        </TabsContent>
...
export default function MyCoursesPage() {
  ...
  const enrollmentsQuery = useGetApiEnrollmentsMe({
  ...
  const isLoading =
    ...
    overviewQueries.some((query) => query.isLoading);
  const failing = classes.filter(
    (course) => calcRate(course) < FAIL_THRESHOLD && course.totalModules > 0,
  );
