...
import {
  ArrowLeft, CalendarCheck, CheckCircle2, Clock, ExternalLink,
  Loader2, Nfc, QrCode, RefreshCw, StopCircle, Users, Wifi, XCircle,
} from "lucide-react"
...
import { useGetApiSessions, usePostApiSessionsIdClose } from "@/lib/api/sessions/sessions"
import { AttendanceMethod, AttendanceStatus } from "@/lib/api/model"
import type { SessionDto } from "@/lib/api/model"
import { MOCK_SECTIONS, MOCK_SESSIONS, makeMockEnrollments } from "@/lib/mock-data"
...
function textColor(v: number) {
  return v >= 80 ? "text-primary" : v >= 60 ? "text-amber-500" : "text-destructive"
}
...
function SessionDetail({ session }: { session: SessionDto }) {
  ...
  const [qrToken, setQrToken] = useState(() => `att://session/${session.id}?ts=${Date.now()}`)
  const { mutateAsync: closeSession, isPending: isClosing } = usePostApiSessionsIdClose()
  ...
  const { data: moduleData } = useGetApiModulesId(session.moduleId, { query: { enabled: !mockMode } })
  ...
  const mockOfferingId = mid >= 100 && mid < 200 ? 9001 : mid >= 200 && mid < 300 ? 9002 : 9003
  const mockSections = MOCK_SECTIONS[String(mockOfferingId)] ?? []
  const enrollments = mockMode ? makeMockEnrollments(mockOfferingId, mockSections, 45) : apiEnrollments
  const sectionEnrollments = session.sectionId
    ? enrollments.filter((enrollment) => String(enrollment.sectionId) === String(session.sectionId))
    : enrollments

  const presentAtts = attendances.filter(
    (attendance) => attendance.status === AttendanceStatus.Present || attendance.status === AttendanceStatus.Late,
  )
  const presentIds = new Set(presentAtts.map((attendance) => String(attendance.userId)))
  const presentList = mockMode
    ? sectionEnrollments.slice(0, Math.round(sectionEnrollments.length * 0.83)).map((enrollment) => ({
    ...
    : presentAtts.map((attendance) => ({
        ...
        via: attendance.method === AttendanceMethod.Manual ? ("manual" as const) : ("auto" as const),
        markedAt: attendance.recordedAt ?? undefined,
      }))

  const presentMockIds = new Set(presentList.map((student) => String(student.id)))
  const absentList = sectionEnrollments
    .filter((enrollment) => mockMode ? !presentMockIds.has(String(enrollment.userId)) : !presentIds.has(String(enrollment.userId)))
    .map((enrollment) => ({ id: enrollment.userId, name: enrollment.userName, studentId: String(enrollment.userId) }))
        ...
        <CardContent className="space-y-4 p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {session.sectionName && <p className="text-sm text-muted-foreground">{session.sectionName}</p>}
              <p className="mt-0.5 text-xs text-muted-foreground">By {session.openedByUserName}</p>
            </div>
            <div className="flex flex-col items-end gap-1.5">
              ...
              <Badge variant={isOpen ? "default" : "secondary"}>{isOpen ? "Open" : "Completed"}</Badge>
            </div>
          </div>
          ...
          {!isLoading && total > 0 && (
            <div className="flex flex-wrap items-center gap-2 pt-1">
              <div className="flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1.5 text-xs font-medium text-primary">
                <CheckCircle2 className="h-3.5 w-3.5" />{presentList.length} present
              </div>
              <div className="flex items-center gap-1.5 rounded-full bg-muted px-2.5 py-1.5 text-xs font-medium text-muted-foreground">
                <XCircle className="h-3.5 w-3.5" />{absentList.length} absent
              </div>
              <span className={cn("ml-auto text-xs font-bold tabular-nums", textColor(pct))}>{pct}%</span>
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <CardContent className="p-5">
            <div className="grid gap-4 md:grid-cols-5">
              <div className="flex min-h-72 flex-col items-center justify-center gap-3 rounded-xl border bg-card p-5 md:col-span-3">
                {(session.selectedMethod === AttendanceMethod.Qr || session.selectedMethod === AttendanceMethod.QrWifi) && (
                  <>
                    {session.selectedMethod === AttendanceMethod.QrWifi && (
                      <div className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-blue-500/10 px-3 py-1.5 text-xs text-blue-500">
                        <Wifi className="h-3.5 w-3.5" />Campus WiFi: <strong className="ml-1 font-mono">IUS-Campus</strong>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-4">
                      <button onClick={refreshQR} className="flex items-center gap-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground">
                        <RefreshCw className="h-3 w-3" />Refresh
                      </button>
                      <button onClick={openQRTab} className="flex items-center gap-1.5 text-xs font-medium text-primary transition-colors hover:text-primary/80">
                ...
                {session.selectedMethod === AttendanceMethod.Nfc && (
                  <div className="flex flex-col items-center gap-4">
                    <div className="relative flex h-28 w-28 items-center justify-center">
                      <div className="absolute inset-0 animate-ping rounded-full border-2 border-violet-500/15" style={{ animationDuration: "2s" }} />
                    ...
                    <div className="text-center">
                      ...
                      <p className="mt-0.5 text-xs text-muted-foreground">Students tap their student card</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="overflow-hidden rounded-xl border bg-card md:col-span-2">
                <div className="flex items-center justify-between border-b bg-muted/30 px-3 py-2">
                  <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Live Attendance</span>
                  <span className="text-xs text-muted-foreground">{presentList.length}/{sectionEnrollments.length}</span>
                </div>
                <div className="max-h-72 divide-y overflow-y-auto">
                  {presentList.map((student) => (
                    <div key={String(student.id)} className="flex items-center gap-2 px-3 py-2">
                  ...
                  {absentList.map((student) => (
                    <div key={String(student.id)} className="flex items-center gap-2 px-3 py-2">
                      ...
                      <span className="truncate text-xs text-muted-foreground">{student.name}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-4 flex justify-end">
              <Button variant="destructive" size="sm" onClick={handleEndSession} disabled={isClosing || mockMode} className="gap-2">
                {isClosing ? <Loader2 className="h-4 w-4 animate-spin" /> : <StopCircle className="h-4 w-4" />}
                End Session
              </Button>
            </div>
          </CardContent>
  ...
  if (!session) {
    return (
      <div className="mx-auto max-w-screen-md space-y-4">
        <button onClick={() => router.push("/attendance")} className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground">
          <ArrowLeft className="h-4 w-4" />Back to attendance
        </button>
