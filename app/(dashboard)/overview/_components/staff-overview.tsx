...
const TODAY_SCHEDULE = [
  {
    ...
    time: "09:00 - 10:30",
    code: "CS101",
    module: "Module 4: Loops & Iteration",
    section: "Section A",
    students: 28,
    status: "done",
    present: 24,
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <Badge
                      className={cn(
                        "text-[10px] px-1.5 py-0 ml-auto border-0",
                        session.status === "done"
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-bold">
                    {session.status === "done" ? `${session.present}/` : ""}
                    {session.students}
                  </p>
                  <p className="text-[10px] text-muted-foreground">
                    {session.status === "done" ? "present" : "students"}
                  </p>
                </div>
              </div>
            ))}
              ...
              <div key={c.id} className="space-y-2">
                ...
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div
                    className={cn(
                      "h-full rounded-full",
                      barColor(c.attendance),
                    )}
                    style={{ width: `${c.attendance}%` }}
                  />
                </div>
                <p className="text-[11px] text-muted-foreground">
                  {c.students} students · {c.term}
                </p>
              </div>
            ))}
          </div>
